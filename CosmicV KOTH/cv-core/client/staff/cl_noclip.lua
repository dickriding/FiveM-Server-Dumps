...
local function toggleNoclip()
	if noClipEnabled == false then return end
	...
	CreateThread(function()
		while inputRotEnabled do
			...
			if (math.abs(rightAxisX) > 0) and (math.abs(rightAxisY) > 0) then
				...
				local rotz = rotation.z + rightAxisX * -10.0
...
local function toggleNoClipMode(forceMode)
	...
	LocalPlayer.state:set("noclip", noClipEnabled, true)
	--TODO: Notify staff member of noclip status
end

RegisterNetEvent('cv-core:ToggleNoClip', function(forceMode)
	toggleNoClipMode(forceMode)
end)
