...
RegisterNUICallback('chatResult', function(data, cb)
  ...
  if not data.canceled then
    ...
    if string.len(data.message) >= 300 then return end
...
RegisterNUICallback('loaded', function(data, cb)
  ...
  SendNUIMessage({
    type = 'ON_UPDATE_EMOTES',
    emotes = GlobalState.chatEmotes
  })

  cb('ok')
end)

AddStateBagChangeHandler("chatEmotes", "global", function(_, key, value, _, _)
