...
CreateThread(function()
  local last_active = chatInputActive

  while true do
    local changed = false

    if(chatInputActive ~= last_active) then
      changed = true

      TriggerEvent("chat:typing", chatInputActive)
      TriggerServerEvent("chat:typing", chatInputActive)
      last_active = chatInputActive
    end
...
RegisterNUICallback('chatResult', function(data, cb)
  ...
  if not data.canceled then
    ...
    if data.message:sub(1, 1) == '/' then
      ...
      TriggerServerEvent('_chat:messageEntered', GetPlayerName(id), { r, g, b }, data.message:gsub("\n", " "), data.mode)
    end
  end
  ...
  while true do
    ...
    if chatLoaded then
      local forceHide =
        ...
        IsPlayerSwitchInProgress() or
        IsHelpMessageBeingDisplayed() or
