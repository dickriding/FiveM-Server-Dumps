    ...
    add('spawnpoint', function(state, model)
        ...
        return function(opts)
            ...
            local s, e = pcall(function()
                ...
                if(model == "__rsm_random") then
                    while not IsModelInCdimage(GetHashKey(model)) do
                        if(model ~= "__rsm_random") then
                            print("Invalid model: " .. model)
                        end

                        model = appropriateModels[math.random(1, #appropriateModels)]
                    end
                end
...
function addSpawnPoint(spawn)
    ...
    if not IsModelInCdimage(model) then
        error("invalid spawn model:" .. modelStr)
    end
