...
-- API
function WarMenu.CreateMenu(id, title, closeCallback)
    -- Default settings
    menus[id] = { }
...
function WarMenu.CreateSubMenu(id, parent, subTitle, closeCallback)
...
function WarMenu.IsAnyMenuWithTitleOpened(subtitle)
...
function WarMenu.IsThisMenuAboutToBeClosed(id)
    if menus[id] then
...
function WarMenu.DisabledButton(text, subText)
    local buttonText = text
    if subText then
        buttonText = '{ '..tostring(buttonText)..', '..tostring(subText)..' }'
    end
...
function WarMenu.SpriteButton(text, textDict, sprite, focusSprite)
    local buttonText = text
...
function WarMenu.SpriteMenuButton(text, textDict, sprite, focusSprite, id)
    if menus[id] then
        local clicked, hovered = WarMenu.SpriteButton(text, textDict, sprite, focusSprite)
        if clicked then
            setMenuVisible(currentMenu, false)
...
function WarMenu.ComboBox(text, items, currentIndex, selectedIndex, callback, displaycb)
...
function WarMenu.MenuButton(text, id, secondtext)
    if menus[id] then
		local clicked, hovered = WarMenu.Button(text, (secondtext and secondtext or "→")) 
		if clicked then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
...
function WarMenu.SwitchMenu(id)
	setMenuVisible(currentMenu, false)
    setMenuVisible(id, true, true)
end

function WarMenu.SetDescription(text)
...
function WarMenu.CheckBox(text, bool, callback)
    ...
    if WarMenu.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
...
function WarMenu.UseSpriteAsBackground(id, textDict, sprite, r, g, b, a, stillDrawText)
