...
local function startTargeting()
    ...
    while state.isActive() do
        ...
        --[[if entityHit ~= lastEntity then
            if lastEntity and DoesEntityExist(lastEntity) and GetEntityType(lastEntity) ~= 1 then
                SetEntityDrawOutline(lastEntity, false)
                SetEntityDrawOutlineColor(0, 0, 0, 0)
            end

            if entityType ~= 1 and GetEntityType(entityHit) > 1 then
                SetEntityDrawOutline(entityHit, true)
                SetEntityDrawOutlineColor(255, 255, 255, 255 / 4)
            end
        end]]
