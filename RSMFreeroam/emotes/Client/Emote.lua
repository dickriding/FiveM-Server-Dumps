...
RegisterCommand('-emotemenu', function() end, false)
RegisterKeyMapping('+emotemenu', 'Emotes - Open Menu', 'keyboard', 'K')

RegisterCommand('+favemote', function()
...
RegisterCommand('-favemote', function() end, false)
RegisterKeyMapping('+favemote', 'Emotes - Play favourite', 'keyboard', 'CAPITAL')

RegisterCommand('+cancelemote', function() EmoteCancel() end, false)
RegisterCommand('-cancelemote', function() end, false)
RegisterKeyMapping('+cancelemote', 'Emotes - Cancel', 'keyboard', 'X')
...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/emotemenu', 'Open the emotes menu.')
    TriggerEvent('chat:addSuggestion', '/emotes', 'List available emotes.')
    TriggerEvent('chat:addSuggestion', '/walk', 'Set your walking style.', {{ name="style", help="/walks for a list of valid styles"}})
...
function EmoteMenuStart(args, hard)
    ...
    elseif(etype == "custom") then
      if DP.Custom[name] ~= nil then
        if OnEmotePlay(DP.Custom[name]) then end
      end
...
function EmoteCommandStart(source, args, raw)
    ...
    elseif DP.Custom[name] ~= nil then
      if OnEmotePlay(DP.Custom[name]) then end return
...
function OnEmotePlay(EmoteName)
  ...
  local cooldown = (lobby and lobby.flags) and lobby.flags.emote_cooldown or 0

  -- Cooldown check
  if GetGameTimer() - lastEmote < cooldown then
    return
  end

  -- update the last emote time
  lastEmote = GetGameTimer()
  ...
  -- CTaskDamageElectric (Taser)
  if GetIsTaskActive(PlayerPedId(), 26) then
    return
  end

  -- CTaskGetUp
  if GetIsTaskActive(PlayerPedId(), 16) then
    return
  end

  -- CTaskGetUpAndStandStill
  if GetIsTaskActive(PlayerPedId(), 17) then
    return
  end

  -- CTaskFallOver
  if GetIsTaskActive(PlayerPedId(), 18) then
    return
  end

  -- CTaskFallAndGetUp
  if GetIsTaskActive(PlayerPedId(), 19) then
    return
  end
