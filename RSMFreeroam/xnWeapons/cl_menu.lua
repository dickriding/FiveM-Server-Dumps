...
local function drawTitle()
    if menus[currentMenu] then
		...
		elseif menus[currentMenu].titleFont == "~sprite~" then
            ...
            if HasStreamedTextureDictLoaded("shopui_title_gunclub") then
                SetUiLayer(0)
                DrawSprite("shopui_title_gunclub", "shopui_title_gunclub", x, y, menuWidth, titleHeight, 0.0, 255, 255, 255, 255, 0)
            end
...
function JayMenu.MenuButton(text, id, secondtext)
    if menus[id] then
        if JayMenu.Button(text, (secondtext and secondtext or "→")) then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
