...
RegisterCommand("manual", function()
    ...
    TriggerEvent("chat:addMessage", {
        ...
        multiline = true,
        args = {
            ("[^3RSM^7] Transmission has been set to %s^7."):format(manualon and "^1Manual" or "^2Auto")
        }
    })
end)
    ...
    if vehicle ~= nil then
        if manualon == false then
            TriggerEvent("chat:addMessage", {
                ...
                multiline = true,
                args = {
                    ("[^3RSM^7] You are not in Manual Transmission Mode, this cannot be toggled.")
                }
            })
        else
            if realistic then
            ...
            TriggerEvent("chat:addMessage", {
                ...
                multiline = true,
                args = {
                    ("[^3RSM^7] Manual Transmission Mode has been set to %s^7."):format(realistic and "^2Realistic" or "^1Arcade")
                }
            })
        end
    end
    ...
    Citizen.CreateThread(function ()
        if manualon and vehicle ~= nil and ready then
            if selectedgear <= numgears - 1 then 
                ...
                if selectedgear > numgears then selectedgear = numgears end
    ...
    Citizen.CreateThread(function ()
        if manualon and vehicle ~= nil and ready then
            if selectedgear > -1 then
                ...
                Wait(300)
                selectedgear = selectedgear - 1
                if selectedgear < -1 then selectedgear = -1 end
                DisableControlAction(0, 71, false)
                SimulateGears()
            end
        end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0) 
        if manualon and vehicle ~= nil then
            local isController = not IsInputDisabled(2)
            if isController then
                DisableControlAction(0, Config.controller.shiftUp, true)
                DisableControlAction(0, Config.controller.shiftDown, true)

                if IsDisabledControlJustPressed(0, Config.controller.shiftUp) then
                    doShiftUp()
                elseif IsDisabledControlJustPressed(0, Config.controller.shiftDown) then
                    doShiftDown()
                end
            else
...
RegisterCommand('shiftup', doShiftUp)
RegisterCommand('shiftdown', doShiftDown)

RegisterKeyMapping('shiftup', 'Shift up', 'keyboard', 'LSHIFT')
RegisterKeyMapping('shiftdown', 'Shift down', 'keyboard', 'R')
...
Citizen.CreateThread(function() --? Handle gear changes
    while true do
        Citizen.Wait(0)
        if manualon and vehicle ~= nil then
            if selectedgear == -1 then
...
local disable = false
Citizen.CreateThread(function() --? Enable "realism" mode
    while true do
        Citizen.Wait(0)
        if realistic then
            if manualon and vehicle ~= nil then
                if selectedgear > 1 then
                    if IsControlPressed(0,71) then
                        ...
                        if speed < minspeed then
                            if GetVehicleCurrentRpm(vehicle) < 0.4 and not disable then
                                disable = true
                                Citizen.CreateThread(function()
...
Citizen.CreateThread(function() --? Handle idle
    local stopTime = nil
    while true do
        Citizen.Wait(100)
        if manualon and DoesEntityExist(vehicle) and ready and selectedgear > 0 then
            local newSpeed = GetEntitySpeed(vehicle)
            if IsVehicleStopped(vehicle) then
                if stopTime == nil then
                    stopTime = GetGameTimer()
                end
                
                if GetGameTimer() - stopTime > 3000 then
                    selectedgear = 0
                    SimulateGears()
                    stopTime = nil
                end
...
--         SetTextEntry("STRING")
--         if manualon then
--             AddTextComponentString("Gear: ~p~"..selectedgear.." ~s~Controller: "..(IsInputDisabled(2) and "~r~No" or "~g~Yes"))
--         end
--         DrawText(0.4, 0.020)
--     end
-- end)
...
exports("getCarGear",function(playerCar)
    return manualon and tostring(getinfo(selectedgear)) or false
end)
