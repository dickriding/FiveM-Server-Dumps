function ComputeBowlingScore(totalRoundCount, bowlingThrows)
    ...
    for i = 1, math.min(#bowlingThrows, totalRoundCount) do
        if bowlingThrows[i] then
            ...
            if
                firstThrow ~= nil and (secondThrow ~= nil or firstThrow == 10) and
                    (i < totalRoundCount or thirdThrow ~= nil)
             then
                thisRoundSum =
                    (firstThrow and firstThrow or 0) + (secondThrow and secondThrow or 0) +
                    (thirdThrow and thirdThrow or 0)
                        ...
                        if bowlingThrows[i + 1] ~= nil then
                            if bowlingThrows[i + 1][1] ~= nil then
                                ...
                                if bowlingThrows[i + 1][1] == 10 then
                                    if
                                        i == (totalRoundCount - 1) and bowlingThrows[i + 1][2] ~= nil and
                                            bowlingThrows[i + 1][3] ~= nil
                                     then
...
function GetCurrentTurnPlayerServerId(laneState, totalRounds)
    if laneState == nil or #laneState < 1 or #laneState[1].Throws == nil then
        return nil, nil
    end
