  ...
  function(data, cb)
    ...
    if not data.canceled then
      ...
      if data.message:sub(1, 1) == "/" then
        TriggerServerEvent("_chat:log", GetPlayerName(id), data.message)
  ...
  function()
    ...
    while true do
      ...
      if not chatInputActive then
        if
          (IsControlPressed(0, 245) or (IsDisabledControlPressed(0, 245) and IsPedDeadOrDying(PlayerPedId(), 1))) and
            not exports.phone:isOpen()
         then
