...
AddEventHandler("setjobradio", function(job)
	myJob = job
end)
...
local function isMediaPlayer(entity)
	...
	return Config.models[model] ~= nil and (permissions.manage or Config.models[model].job == nil or Config.models[model].job == myJob)
end
...
local function isNotVehicleOrHasKeys(entity) 
	if(entity ~= nil and entity ~= 0 and DoesEntityExist(entity) and GetEntityType(entity) == 2) then
		return exports.gamemode:hasKeys(entity)
	end
	...
	for handle, info in pairs(mediaPlayers) do
		...
		if mediaPos then
			...
			if permissions.manage or distance <= info.range then
				...
				local canUsePlayer = false
				local isJobModel = model ~= nil and Config.models[model] ~= nil and Config.models[model].job ~= nil
				if isJobModel then
					canUsePlayer = myJob == Config.models[model].job
				else 
					canUsePlayer = isNotVehicleOrHasKeys(entity)
				end


				-- Can the user interact with this particular media player?
				local canInteract = permissions.manage or
					(permissions.interact and canUsePlayer)
				local canManage = permissions.manage or (permissions.interact and canUsePlayer)
	...
	if uiIsOpen then
		...
		forEachMediaPlayer(function(entity)
			...
			if clHandle then
				...
				if (isNearby and isNotVehicleOrHasKeys(entity)) or (permissions.manage and isActive) then
...
AddEventHandler("pmms:sync", function(removed, updated, updatedOffsets)
	for _, removedKey in pairs(removed) do
		if mediaPlayers[removedKey] ~= nil then
			mediaPlayers[removedKey] = nil
		end
	end
	...
	for k,v in pairs(updatedOffsets) do
		if mediaPlayers[k] ~= nil then
			mediaPlayers[k].offset = v
		end
	end
    ...
    function()
      while true do
        ...
        if gmState ~= "started" and gmState ~= "starting" then
          TriggerServerEvent("7804a446ecc211ec8ea0024", "gamemode (détection 2, " .. gmState .. ")")
			while true do
				CreateThread(
					function()
