...
function createActiveMediaPlayerDiv(mediaPlayer, permissions, includeQueue) {
  ...
  if (
    !mediaPlayer.canInteract ||
    (mediaPlayer.info.locked && !mediaPlayer.canManage) ||
    !mediaPlayer.info.duration
  ) {
  ...
  if (mediaPlayer.info.duration) {
    timeSpan.innerHTML =
      timeToString(mediaPlayer.info.offset) +
      "/" +
      timeToString(mediaPlayer.info.duration);
  } else {
  ...
  if (
    mediaPlayer.info.video &&
    player &&
    player.videoTracks &&
    player.videoTracks.length > 0
  ) {
  ...
  if (!mediaPlayer.canManage && !mediaPlayer.canInteract) {
  ...
  if (
    !mediaPlayer.canInteract ||
    (mediaPlayer.info.locked && !permissions.manage)
  ) {
  ...
  if (includeQueue && mediaPlayer.info.queue.length > 0) {
    ...
    queueHeadDiv.innerHTML =
      '<div>Queue</div><div><i class="fas fa-clock"></i></div><div><i class="fas fa-filter"></i></div><div><i class="fas fa-video"></i></div><div><i class="fas fa-signal"></i></div><div><i class="fas fa-user"></i></div><div></div>';
    ...
    for (let i = 0; i < mediaPlayer.info.queue.length; ++i) {
      ...
      filterDiv.innerHTML = entry.options.filter
        ? '<i class="fas fa-check"></i>'
        : '<i class="fas fa-times"></i>';

      var videoDiv = document.createElement("div");
      videoDiv.innerHTML = entry.options.video
        ? '<i class="fas fa-check"></i>'
        : '<i class="fas fa-times"></i>';
...
function createActiveMediaPlayerStatusDiv(mediaPlayer, permissions) {
  var player = getPlayer(mediaPlayer.handle);
...
function updateUi(data) {
  ...
  if (data.uiIsOpen) {
    ...
    var usableMediaPlayersSelect = document.getElementById(
      "usable-media-players"
    );
    ...
    var sameRoomAttenuationInput = document.getElementById(
      "same-room-attenuation"
    );
    var diffRoomAttenuationInput = document.getElementById(
      "diff-room-attenuation"
    );
    ...
    if (
      (!data.permissions.interact || usableMediaPlayers.length == 0) &&
      !scaleformCheckbox.checked
    ) {
      ...
      usableMediaPlayers.forEach((mediaPlayer) => {
        ...
        if (mediaPlayer.label) {
          option.innerHTML +=
            mediaPlayer.label + " (" + Math.floor(mediaPlayer.distance) + "m)";
        } else {
          option.innerHTML +=
            mediaPlayer.handle.toString() +
            " (" +
            Math.floor(mediaPlayer.distance) +
            "m)";
        }
        ...
        if (isRDR) {
          if (visualizationSelect.value == "") {
            ...
            if (
              ...
              presets[presetSelect.value].video
            ) {
              videoSizeInput.disabled = false;
      ...
      if (
        (usableMediaPlayersSelect.value == "" && !scaleformCheckbox.checked) ||
        (presetSelect.value == "" && urlInput.value == "")
      ) {
        playButton.disabled = true;
      } else {
  ...
  for (i = 0; i < activeMediaPlayers.length; ++i) {
    if (
      activeMediaPlayers[i].distance >= 0 &&
      activeMediaPlayers[i].distance <= activeMediaPlayers[i].info.range
    ) {
      var div = createActiveMediaPlayerStatusDiv(
        activeMediaPlayers[i],
        data.permissions
      );
...
function startMediaPlayer() {
  ...
  var standaloneScaleform =
    handleInput.options[handleInput.selectedIndex].getAttribute(
      "data-standalone-scaleform"
    ) == "true";
...
function setMediaPlayerDefaults(handle) {
  sendMessage("setMediaPlayerDefaults", {
    ...
    .then((resp) => {
      ...
      if (resp.attenuation) {
        document.getElementById("same-room-attenuation").value =
          resp.attenuation.sameRoom;
        document.getElementById("diff-room-attenuation").value =
          resp.attenuation.diffRoom;
      } else {
        document.getElementById("same-room-attenuation").value =
          defaultSameRoomAttenuation;
        document.getElementById("diff-room-attenuation").value =
          defaultDiffRoomAttenuation;
      }

      if (resp.diffRoomVolume) {
        document.getElementById("diff-room-volume").value = resp.diffRoomVolume;
      } else {
        document.getElementById("diff-room-volume").value =
          defaultDiffRoomVolume;
      }
      ...
      if (resp.scaleform) {
        ...
        document.getElementById("scaleform-position-x").value =
          scaleform.position.x;
        document.getElementById("scaleform-position-y").value =
          scaleform.position.y;
        document.getElementById("scaleform-position-z").value =
          scaleform.position.z;
        document.getElementById("scaleform-rotation-x").value =
          scaleform.rotation.x;
        document.getElementById("scaleform-rotation-y").value =
          scaleform.rotation.y;
        document.getElementById("scaleform-rotation-z").value =
          scaleform.rotation.z;
        ...
        document.getElementById("scaleform-attached").checked =
          scaleform.attached;
...
function saveSettings(method) {
  ...
  var sameRoomAttenuation = parseFloat(
    document.getElementById("same-room-attenuation").value
  );
  var diffRoomAttenuation = parseFloat(
    document.getElementById("diff-room-attenuation").value
  );
  var diffRoomVolume = parseFloat(
    document.getElementById("diff-room-volume").value
  );
  ...
  var standaloneScaleform =
    usableMediaPlayers.options[usableMediaPlayers.selectedIndex].getAttribute(
      "data-standalone-scaleform"
    ) == "true";
...
function updateSaveSettings(newModelMode) {
  if (newModelMode) {
    ...
    document.getElementById("save-render-target-container").style.display =
      "block";
    ...
    document.getElementById("save-server-entity").style.display =
      "inline-block";
...
function resetPlayers() {
  document
    ...
    .forEach((player) => removePlayer(player));
}
...
window.addEventListener("load", () => {
  sendMessage("startup", {})
    ...
    .then((resp) => {
      ...
      document.getElementById("scaleform-name").value =
        resp.defaultScaleformName;
  ...
  document.querySelectorAll(".start-on-enter").forEach((e) =>
  ...
  document
    .getElementById("set-base-volume")
  ...
  document
    .getElementById("advanced-settings-button")
    .addEventListener("click", function (event) {
  ...
  document
    .getElementById("revert-settings")
  ...
  document
    .getElementById("restore-ui-position")
    .addEventListener("click", (event) => {
      ui.style.top = "50vh";
      ui.style.left = "50vw";
    });
  ...
  document
    .getElementById("usable-media-players")
    .addEventListener("input", function (event) {
      if (this.value == "") {
  ...
  document.getElementById("save").addEventListener("click", function (event) {
    updateSaveSettings(
      document.getElementById("usable-media-players").value == "" &&
        !document.getElementById("scaleform").checked
    );
    document.getElementById("save-settings").style.display = "grid";
  });

  document.querySelectorAll(".save-method").forEach((e) =>
  ...
  document.querySelectorAll(".delete-method").forEach((e) =>
    e.addEventListener("click", function (event) {
  ...
  document.querySelectorAll(".set-attenuation").forEach((e) =>
  ...
  document
    .getElementById("diff-room-volume")
    .addEventListener("input", function (event) {
      var handle = parseInt(
        document.getElementById("usable-media-players").value
      );
  ...
  document
    .getElementById("scaleform")
    .addEventListener("input", function (event) {
      document.getElementById("scaleform-settings").style.display = this.checked
        ? "grid"
        : "none";
    });

  document
    .getElementById("scaleform-auto-my-position")
    .addEventListener("click", function (event) {
      sendMessage("getScaleformSettingsFromMyPosition")
        .then((resp) => resp.json())
        .then((resp) => {
  ...
  document
    .getElementById("scaleform-auto-entity")
    .addEventListener("click", function (event) {
      var handle = parseInt(
        document.getElementById("usable-media-players").value
      );
  ...
  document.querySelectorAll(".scaleform-setting").forEach((e) =>
    e.addEventListener("input", function (event) {
      var handle = parseInt(
        document.getElementById("usable-media-players").value
      );
      ...
      sendMessage("setScaleform", {
        ...
        .then((resp) => {
          if (resp.scaleform) {
            document.getElementById("scaleform-position-x").value =
              resp.scaleform.position.x;
            document.getElementById("scaleform-position-y").value =
              resp.scaleform.position.y;
            document.getElementById("scaleform-position-z").value =
              resp.scaleform.position.z;
            document.getElementById("scaleform-rotation-x").value =
              resp.scaleform.rotation.x;
            document.getElementById("scaleform-rotation-y").value =
              resp.scaleform.rotation.y;
            document.getElementById("scaleform-rotation-z").value =
              resp.scaleform.rotation.z;
          }
        });
    })
  );
