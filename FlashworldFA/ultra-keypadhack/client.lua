...
function showTimer()
	if (timeString == nil or #timeString == 0) then
		return
	end
	drawSprite("mphackinggame", "numbers_" .. timeString[1], 0.258, 0.152, 0.025, 0.055, 0, 255, 255, 255, 255)

	drawSprite("mphackinggame", "numbers_" .. timeString[2], 0.283, 0.152, 0.025, 0.055, 0, 255, 255, 255, 255)

	drawSprite("mphackinggame", "numbers_colon", 0.307, 0.154, 0.025, 0.055, 0, 255, 255, 255, 255)

	drawSprite("mphackinggame", "numbers_" .. timeString[3], 0.33, 0.153, 0.025, 0.055, 0, 255, 255, 255, 255)

	drawSprite("mphackinggame", "numbers_" .. timeString[4], 0.355, 0.153, 0.025, 0.055, 0, 255, 255, 255, 255)
end
	...
	CreateThread(
		function()
			...
			while isGameActive do
				...
				drawSprite("mphackinggamebg", "bg", 0.50, 0.50, 1.0, 1.0, 0, 255, 255, 255, 255)

				-- Animation stuff
				if toggleAnimOverlay then
					drawSprite("mphackinggamewin", "tech_1_3", 0.2, 0.40, 0.3, 0.5, 0, 255, 255, 255, 255)

					drawSprite("mphackinggamewin2", "tech_2_2", 0.18, 0.80, 0.3, 0.3, 0, 255, 255, 255, 255)

					drawSprite("mphackinggamewin", "tech_1_1", 0.8, 0.50, 0.3, 0.6, 0, 0, 140, 150, 255)
				else
					drawSprite("mphackinggamewin", "tech_1_1", 0.2, 0.40, 0.3, 0.5, 0, 255, 255, 255, 255)

					drawSprite("mphackinggamewin2", "tech_2_0", 0.18, 0.80, 0.3, 0.3, 0, 255, 255, 255, 255)

					drawSprite("mphackinggamewin", "tech_1_4", 0.8, 0.50, 0.3, 0.6, 0, 0, 140, 150, 255)
				end

				-- Draw main overlay over background
				drawSprite("mporderunlock", "background_layout", 0.50, 0.50, 0.7, 0.85, 0, 255, 255, 255, 255)

				-- More animation stuff
				if toggleAnimOverlay then
					drawSprite("mporderunlock_decor", "techaration_0", 0.50, 0.50, 0.7, 0.85, 0, 255, 255, 255, 255)
				else
					drawSprite("mporderunlock_decor", "techaration_1", 0.50, 0.50, 0.7, 0.85, 0, 255, 255, 255, 255)
				end

				-- Life counter
				for i = 1, availableLife do
					drawSprite("mphackinggame", "life", lifePos[i].x, lifePos[i].y, 0.035, 0.055, 0, 255, 255, 255, 255)
				end
				...
				if showFakePattern then
					if curFakePattern ~= nil then
						for i, k in pairs(curFakePattern) do
							drawSprite("mporderunlock", "correct_circles", k.x, k.y, 0.06, 0.10, 0, 255, 255, 255, 255)
						end
					end
				end
				...
				if activeColumn > 0 and activeRow > 0 then
					drawSprite(
						"mporderunlock",
						"selector",
						innerCircles[activeColumn][activeRow].x,
						innerCircles[activeColumn][activeRow].y,
						0.07,
						0.11,
						0,
						255,
				...
				if activeStage < 5 then
					drawSprite(
						"mporderunlock",
						"keypad_feedback_box",
						resultCodePos[activeStage].x,
						resultCodePos[activeStage].y,
						0.045,
						0.07,
						0,
						255,
				...
				for i = 1, #selectedDots["correctDots"] do
					...
					drawSprite("mporderunlock", "correct_circles", pos.x, pos.y, 0.06, 0.10, 0, 255, 255, 255, 255)
				end
				...
				for i = 1, #selectedDots["incorrectDots"] do
					...
					local pos = innerCircles[colNum][rowNum]
					drawSprite("mporderunlock", "incorrect_circles", pos.x, pos.y, 0.02, 0.03, 0, 255, 255, 255, 255)
				end
					...
					elseif IsControlJustPressed(0, 191) then -- Enter
						if activeColumn > 0 and activeColumn < 7 and activeRow ~= 0 then
							if passcodePattern[activeColumn] == activeRow then -- If the selected position is correct then
								...
								if activeColumn == 6 and activeStage < 2 then
									blinkDialog(true)
									showFakePattern = false
									curFakePattern = {}
									activeColumn = 0
								...
								elseif activeColumn == 6 and activeStage == 2 then
									activeStage = activeStage + 1
									activeRow = 0
									activeColumn = 0
									isPlayerInputDisabled = true
