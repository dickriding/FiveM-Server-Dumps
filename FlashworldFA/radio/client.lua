NETWORK_PROP = (GetConvarInt("IS_DEV", 0) == 1) or (GetConvar("SVR_NAME", "ADV") == "ADV")
							...
							radioConfig.Controls.Increase.Name ..
								"~ pour changer de fréquence~n~~" ..
									radioConfig.Controls.Input.Name ..
										"~ choisir la fréquence~n~~" ..
											radioConfig.Controls.ToggleClicks.Name .. "~ Pour ~r~désactiver~w~ les sons radio~n~fréquence: ~1~ MHz"
	},
	{
		"FRZL_RADIO_HELP2",
		-- "~s~" ..
		...
		"~" ..
			radioConfig.Controls.Toggle.Name ..
				"~ Pour ~r~éteindre ~s~la radio.~n~~INPUT_C84E0235~ Pour parler.~n~fréquence: ~1~ MHz"
	},
						...
						"~ ou ~" ..
							radioConfig.Controls.Increase.Name ..
								"~ pour changer de fréquence~n~~" ..
									radioConfig.Controls.Input.Name ..
										"~ choisir la fréquence~n~~" ..
											radioConfig.Controls.ToggleClicks.Name .. "~ Pour ~g~activer~w~ les sons radio~n~fréquence: ~1~ MHz"
	}
}
...
Radio.Commands = {
	{
		...
		Help = "Help voc", -- Command help shown in chatbox when typing the command
		Params = {},
		Handler = function(src, args, raw)
			...
			elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
				...
				Radio.On = false
				Radio:Remove()
				exports["vocal"]:setVoiceProperty("radioEnabled", false)
	...
	},
	{
		...
		Name = "fréquence", -- Command name
		Help = "Changer la fréquence radio", -- Command help shown in chatbox when typing the command
		Params = {
			...
			if Radio.Has then
				if args[1] then
					...
					if newFrequency then
						...
						if
							newFrequency >= minFrequency and newFrequency <= radioConfig.Frequency.List[#radioConfig.Frequency.List] and
								newFrequency == math.floor(newFrequency)
						 then
							if not radioConfig.Frequency.Private[newFrequency] or radioConfig.Frequency.Access[newFrequency] then
								...
								if idx ~= nil then
									...
									SetResourceKvpInt("ADV_RADIO_CURRENT", newFrequency)
...
RegisterKeyMapping("radio", "Menu radio (item nécessaire)", "keyboard", "F1")
...
function Radio:Toggle(toggle)
	...
	if not self.Has or IsEntityDead(playerPed) then
		...
		exports.gamemode:delete(self.Handle)
	...
	if self.On and not radioConfig.AllowRadioWhenClosed then
		exports["vocal"]:setVoiceProperty("radioEnabled", toggle)
	end
	...
	if self.Open then
		...
		local timeout = GetGameTimer() + 5000
		while not HasModelLoaded(self.Prop) and GetGameTimer() < timeout do
			Citizen.Wait(150)
			RequestModel(self.Prop)
		end

		if not DoesEntityExist(self.Handle) then
			self.Handle = CreateObject(self.Prop, GetEntityCoords(self.Handle), NETWORK_PROP, NETWORK_PROP, false)
		end
...
function Radio:Add(id)
	exports["vocal"]:setRadioChannel(id)
end

-- Remove player from radio channel
function Radio:Remove()
	exports["vocal"]:setRadioChannel(0)
end
...
function Radio:Decrease()
	if self.On then
		if
			radioConfig.Frequency.CurrentIndex - 1 < 1 and
				radioConfig.Frequency.List[radioConfig.Frequency.CurrentIndex] == radioConfig.Frequency.Current
		 then
	...
	SetResourceKvpInt("ADV_RADIO_CURRENT", radioConfig.Frequency.List[radioConfig.Frequency.CurrentIndex])
end
...
function GenerateFrequencyList()
	...
	for i = radioConfig.Frequency.Min, radioConfig.Frequency.Max do
		if (i < 10000 or IsInOrga) and (not radioConfig.Frequency.Private[i] or radioConfig.Frequency.Access[i]) then
...
function SetAllowRadioWhenClosed(value)
	...
	if Radio.On and not Radio.Open and radioConfig.AllowRadioWhenClosed then
		exports["vocal"]:setVoiceProperty("radioEnabled", true)
	end
end
...
local _IsEntityPlayingAnim = IsEntityPlayingAnim
...
Citizen.CreateThread(
	function()
		...
		local savedRadio = GetResourceKvpInt("ADV_RADIO_CURRENT")
		print(current)
		print(savedRadio)
		if savedRadio ~= 0 then
			for k, v in pairs(radioConfig.Frequency.List) do
				if v == savedRadio then
					radioConfig.Frequency.Current = v
					radioConfig.Frequency.CurrentIndex = k
					break
				end
			end
		end
		...
		while true do
			Citizen.Wait(Radio.Open and 0 or 100)
			-- Init local vars
			local playerPed = _PlayerPedId()
			local isFalling = _IsPedFalling(playerPed)
			local isDead = _IsEntityDead(playerPed)
			local minFrequency = radioConfig.Frequency.List[1]
			local broadcastType =
				3 + (radioConfig.AllowRadioWhenClosed and 1 or 0) + ((Radio.Open and radioConfig.AllowRadioWhenClosed) and -1 or 0)
			local broadcastDictionary = Radio.Dictionary[broadcastType]
			local broadcastAnimation = Radio.Animation[broadcastType]
			local isPlayingBroadcastAnim = _IsEntityPlayingAnim(playerPed, broadcastDictionary, broadcastAnimation, 3)
			...
			if
				not radioConfig.Frequency.Access[radioConfig.Frequency.Current] and
					radioConfig.Frequency.Private[radioConfig.Frequency.Current]
			 then
				if Radio.On then
					Radio:Remove()
				end
				...
				SetResourceKvpInt("ADV_RADIO_CURRENT", minFrequency)
			...
			if Radio.Open then
				...
				-- Display help text
				BeginTextCommandDisplayHelp(Radio.Labels[Radio.On and 2 or (Radio.Clicks and 1 or 4)][1])
				...
				if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
					...
					exports["vocal"]:setVoiceProperty("radioEnabled", Radio.On)
				...
				if not Radio.On then
						...
						if IsControlJustPressed(0, radioConfig.Controls.Input.Key) then
							...
							Citizen.CreateThread(
								function()
									...
									if input ~= nil then
										if
											input >= minFrequency and input <= radioConfig.Frequency.List[#radioConfig.Frequency.List] and
										 ...
										 then
											if not radioConfig.Frequency.Private[input] or radioConfig.Frequency.Access[input] then
												...
												if idx ~= nil then
													...
													SetResourceKvpInt("ADV_RADIO_CURRENT", input)
												end
											end
										end
									end
					...
					if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
						...
						exports["vocal"]:setVoiceProperty("micClicks", Radio.Clicks)
					end
				end
			else
...
AddEventHandler(
	...
	function(resName)
		if GetCurrentResourceName() ~= resName and "vocal" ~= resName then
			return
		end

		exports["vocal"]:setVoiceProperty("radioEnabled", false) -- Disable radio control
...
AddEventHandler(
	...
	function(job)
		if tonumber(job) == 1 then
			GivePlayerAccessToFrequencies(1, 2, 3, 4, 5, 6, 21, 22, 23, 24, 25, 26, 30)
		elseif tonumber(job) == 29 then
			GivePlayerAccessToFrequencies(1, 6, 7, 8, 9, 10, 21, 22, 23, 24, 25, 27, 30)
		elseif tonumber(job) == 2 then
			GivePlayerAccessToFrequencies(11, 12, 13, 14, 15, 26, 27, 36)
		elseif tonumber(job) == 28 then
			GivePlayerAccessToFrequencies(16, 17, 18, 19, 20, 26, 27, 36)
		elseif tonumber(job) == 17 then
			GivePlayerAccessToFrequencies(30, 31, 32, 33, 34, 35)
		elseif tonumber(job) == 44 then
			GivePlayerAccessToFrequencies(37, 38, 39, 40)
		elseif tonumber(job) == 39 then
			GivePlayerAccessToFrequencies(41,42)
		end
	end
)
...
AddEventHandler(
	"setorgradio",
	function(org)
		IsInOrga = org ~= nil and tonumber(org) > 0
		GenerateFrequencyList()
	end
)
