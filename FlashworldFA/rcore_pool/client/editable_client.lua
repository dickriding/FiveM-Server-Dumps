    ...
    while true do
        ...
        if not IsCloseToPoolRack and ClosestTableAddress and CurrentState == STATE_NONE then
            if HasPoolCueInHand then
                if not TableData[ClosestTableAddress].player then
                    ...
                    if not WithoutObfuscation(WarMenu).IsMenuOpened('pool') then
                        ...
                        if TableData[ClosestTableAddress] and doesClosestTableHaveBalls then
                            exports.gamemode:displayHelpText('~' .. WithoutObfuscation(Config.Keys.ENTER.label) .. '~ ' .. Config.Text.HINT_PLAY .. '~n~~' .. WithoutObfuscation(Config.Keys.SETUP_MODIFIER.label) .. '~ + ~' .. WithoutObfuscation(Config.Keys.ENTER.label) .. '~' .. costSuffix .. ' ' .. Config.Text.HINT_SETUP)
                        else
                            exports.gamemode:displayHelpText('~' .. WithoutObfuscation(Config.Keys.SETUP_MODIFIER.label) .. '~ + ~' .. WithoutObfuscation(Config.Keys.ENTER.label) .. '~' .. costSuffix .. ' ' .. Config.Text.HINT_SETUP)
                        end
                    end
                ...
                exports.gamemode:displayHelpText(Config.Text.HINT_HINT_TAKE_CUE)
            end
        else
            Wait(500)
        end
