...
function setRadioChannel(channel)
	...
	print(here)
	print(GetResourceKvpInt("ADV_RADIO_HUD_CHANNEL"))
	sendUIMessage(
		{
			radioChannel = hudChannelVisible ~= -1 and channel or "***",
			radioEnabled = radioEnabled
		}
	)
end

hudChannelVisible = GetResourceKvpInt("ADV_RADIO_HUD_CHANNEL") ~= -1
exports(
	"setHudChannelVisible",
	function(visible)
		hudChannelVisible = visible and 1 or -1
		SetResourceKvpInt("ADV_RADIO_HUD_CHANNEL", hudChannelVisible)
		sendUIMessage(
			{
				radioChannel = hudChannelVisible ~= -1 and radioChannel or "***",
				radioEnabled = radioEnabled
			}
		)
	end
...
RegisterCommand(
	...
	function()
		...
		if IsPedCuffed(PlayerPedId()) then
			TriggerEvent(
				"gm:player:localnotify",
				"~r~Action impossible~n~~w~Votre situation ne vous permet pas d'utiliser la radio",
				"",
				3,
				5000
			)
		...
		if (IsPedSwimming(PlayerPedId()) or IsPedSwimmingUnderWater(PlayerPedId())) and not (IsPedInAnyVehicle(PlayerPedId(), false) or exports.gamemode:isWearingScuba()) then
			TriggerEvent(
				"gm:player:localnotify",
				"~r~Action impossible~n~~w~Votre situation ne vous permet pas d'utiliser la radio",
				"",
		...
		if not radioPressed and radioEnabled then
			if radioChannel > 0 then
				...
				playMicClicks(true)
				if
					GetConvarInt("voice_enableRadioAnim", 0) == 1 and
						not (GetConvarInt("voice_disableVehicleRadioAnim", 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false))
				 then
...
if gameVersion == "fivem" then
	RegisterKeyMapping("+talkradio", "Parler dans la radio", "keyboard", "J")
end
