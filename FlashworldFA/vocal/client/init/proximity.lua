...
function orig_addProximityCheck(ply, distance)
	local tgtPed = GetPlayerPed(ply)
	local otherCoords = GetEntityCoords(tgtPed)

	if (#(plyCoords - otherCoords)) > distance then
		return false
	end

	if inInterior then
		return math.abs(plyCoords.z - otherCoords.z) < 2.5
	else
		return true
	end
end
...
function addNearbyPlayers()
	if disableUpdates or IsEntityDead(PlayerPedId()) then
		return
	end
	...
	local distance =
		overrideDistance ~= nil and overrideDistance or
		GetConvar("voice_useNativeAudio", "false") == "true" and voiceModeData[1] * 3 or
		voiceModeData[1]
	...
	for i = 1, #players do
		...
		if ply ~= myPlayer then
			if addProximityCheck(ply, distance) then
...
Citizen.CreateThread(
	function()
		while true do
			...
			Wait(GetConvarInt("voice_refreshRate", 500))
		end
	end
)
...
Citizen.CreateThread(
	function()
		TriggerEvent(
			"chat:addSuggestion",
			"/muteply",
			"Mutes the player with the specified id",
			{
