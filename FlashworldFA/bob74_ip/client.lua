...
Citizen.CreateThread(function()
    ...
    if GetGameBuildNumber() >= 2545 then
        ...
        if GetConvar("SVR_NAME", "ADV") ~= "FLA" then
...
local islandPos = vec(4700.0, -5145.0)
local _SetRadarAsExteriorThisFrame = SetRadarAsExteriorThisFrame
local _SetRadarAsInteriorThisFrame = SetRadarAsInteriorThisFrame
local _Wait = Wait
CreateThread(function()
    while true do
        _SetRadarAsExteriorThisFrame()
        _SetRadarAsInteriorThisFrame(`h4_fake_islandx`, islandPos, 0, 0)
        _Wait(0)
    end
end)
